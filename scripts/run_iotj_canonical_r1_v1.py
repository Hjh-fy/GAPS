...
from gaps_flower.canonical_r1_v1 import (assert_test_access_released, bootstrap_paired_group_deltas,
    ...
    metric_rows_for_slices, summarize_bootstrap,
    select_grouped_cv_alpha, validate_classifier_registry, validate_evidence_bundle,
    validate_access_chronology, validate_r0_bundle)
    ...
    for client in (3, 4, 5):
        for split, sink in (("calibration", calibration), ("test", deferred_test)):
            for suffix in ("features.npy", "phase_labels.npy", "experiment_info.json", "classification_labels.npy", "regression_labels.npy"):
                ...
                if split == "calibration" and sha256(path) != expected:
...
def _source_models():
    return validate_r0_bundle(R0_ROOT, expected_index_sha256=R0_INDEX_SHA256)
...
def formal_run(authorized_head):
    ...
    for target in ("C3", "C4", "C5"):
        ...
        build_feature_cache(DATA_ROOT, cache_root, client=target, split="calibration",
            ...
            extractor_path=ROOT / "run_regression_head_ablation.py", study_id=STUDY_ID, wrapper_path=ROOT/"gaps_flower/canonical_quantitative_features.py")
        for artifact in ("features.npy","phase_labels.npy","experiment_info.json","classification_labels.npy","regression_labels.npy"):
            access.append({"sequence":len(access),"target":target,"split":"calibration","artifact":f"calibration_{artifact}","operation":"load_or_hash"})
        sensor, h1, ids, _ = load_feature_cache(cache_root / target / "calibration",
            expected_dataset_sha256="2f810d7e93cae5f361923184e9dc87d5ae59e0f59be9f52aff7e14f9f33e94f6", expected_study_id=STUDY_ID, expected_wrapper_sha256=sha256(ROOT/"gaps_flower/canonical_quantitative_features.py"), expected_extractor_sha256=sha256(ROOT/"run_regression_head_ablation.py"))
    ...
    lock_hashes={name:sha256(FORMAL_ROOT/name) for name in ("target_alpha_lock.json","target_model_lock.json","classifier_lock.json","bootstrap_design_lock.json")}
    release={"schema_version":"iotj.canonical_v1.r1.test_release.v1","study_id":STUDY_ID,"lock_sha256":lock_hashes,"published_before_sequence":len(access)}
    atomic_json(FORMAL_ROOT / "target_test_release_receipt.json",release)
    ...
    metric_rows, gas_rows, concentration_rows, special_rows, prediction_rows = [], [], [], [], []
    boot_inputs = {}
    for target in ("C3", "C4", "C5"):
        assert_test_access_released(target, locks, access)
        for artifact in ("features.npy","phase_labels.npy","experiment_info.json","classification_labels.npy","regression_labels.npy"):
            access.append({"sequence":len(access),"target":target,"split":"test","artifact":f"test_{artifact}","operation":"load_or_hash"})
        access.append({"sequence":len(access),"target":target,"split":"test","artifact":"classifier_checkpoint","operation":"classifier_inference"})
        ...
        for method,matrix in method_matrices.items():
            repeats=np.asarray([int(d["repeat_id"]) for d in ids]); slices=metric_rows_for_slices(target,method,truth,tc,pc,matrix,ranges,repeats)
            metric_rows += slices["overall"]; gas_rows += slices["gas"]; concentration_rows += slices["concentration"]; special_rows += slices["special"]
            routed=matrix[np.arange(len(tc)),pc]
            for i,d in enumerate(ids): prediction_rows.append({"target":target,"method":method,"sample_index":i,"physical_identity":d["physical_identity"],"filename":d["filename"],"repeat_id":d["repeat_id"],"gas_code":d["gas_code"],"true_class":int(tc[i]),"predicted_class":int(pc[i]),"concentration":float(d["concentration"]),"truth":truth[i],"prediction":routed[i],"gas_range":ranges[int(tc[i])],**{f"route_{g}":float(matrix[i,g]) for g in range(4)}})
    ...
    # Add pooled rows for every registered route/slice family.
    for method in ("SOURCE_ONLY_FEDRIDGE","TARGET_ONLY_83D_RIDGE","R84_CONCAT"):
        subset=[r for r in prediction_rows if r["method"]==method]; y0=np.asarray([float(r["truth"]) for r in subset]); tc0=np.asarray([int(r["true_class"]) for r in subset]); pc0=np.asarray([int(r["predicted_class"]) for r in subset]); mx=np.asarray([[float(r[f"route_{g}"]) for g in range(4)] for r in subset]); rp=np.asarray([int(r["repeat_id"]) for r in subset]); gr={g:float(next(r["gas_range"] for r in subset if int(r["true_class"])==g)) for g in range(4)}; slices=metric_rows_for_slices("POOLED",method,y0,tc0,pc0,mx,gr,rp); metric_rows+=slices["overall"]; gas_rows+=slices["gas"]; concentration_rows+=slices["concentration"]
    ...
    _write_csv(FORMAL_ROOT / "c5_methane_225_repeat1.csv",special_rows)
    bootstrap_rows=[]
    ...
    for target,(y,a,b,g,r,tc) in boot_inputs.items():
        ...
        point_map={"RMSE":point,"MAE":float(np.mean(abs(b-y))-np.mean(abs(a-y))),"NRMSE_range":float(np.sqrt(np.mean(((b-y)/r)**2))-np.sqrt(np.mean(((a-y)/r)**2)))}
        bootstrap_rows += summarize_bootstrap(target,out,point=point_map,n_groups=len(np.unique(g)),n_replicates=5000)
    ...
    point_map={"RMSE":point,"MAE":float(np.mean(abs(b-y))-np.mean(abs(a-y))),"NRMSE_range":float(np.sqrt(np.mean(((b-y)/r)**2))-np.sqrt(np.mean(((a-y)/r)**2)))}
    bootstrap_rows += summarize_bootstrap("POOLED",out,point=point_map,n_groups=len(np.unique(g)),n_replicates=5000)
    all_tc=np.concatenate([v[5] for v in boot_inputs.values()])
    for gas in range(4):
        mask=all_tc==gas; gout=bootstrap_paired_group_deltas(y[mask],a[mask],b[mask],g[mask],r[mask],5000,42)
        gp=float(np.sqrt(np.mean((b[mask]-y[mask])**2))-np.sqrt(np.mean((a[mask]-y[mask])**2))); gas_delta[str(gas)]=gp
        gpmap={"RMSE":gp,"MAE":float(np.mean(abs(b[mask]-y[mask]))-np.mean(abs(a[mask]-y[mask]))),"NRMSE_range":float(np.sqrt(np.mean(((b[mask]-y[mask])/r[mask])**2))-np.sqrt(np.mean(((a[mask]-y[mask])/r[mask])**2)))}
        bootstrap_rows += summarize_bootstrap(f"POOLED_GAS_{gas}",gout,point=gpmap,n_groups=len(np.unique(g[mask])),n_replicates=5000)
    ...
    validate_access_chronology(access,lock_hashes,release)
    atomic_json(FORMAL_ROOT / "DATA_ACCESS_AUDIT.json", {"events":access,"target_test_opened_after_all_locks":True,"release_receipt_sha256":sha256(FORMAL_ROOT/"target_test_release_receipt.json")})
...
def inspect():
    validate_r0_bundle(R0_ROOT, expected_index_sha256=R0_INDEX_SHA256)
