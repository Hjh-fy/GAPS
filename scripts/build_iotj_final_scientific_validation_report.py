...
def build(output: Path) -> dict[str, Any]:
    ...
    claims = [
        ...
        {"Claim": "Main conclusions survive strict non-overlap", "Evidence": "raw-file-disjoint A4+R84 sensitivity", "Protocol": "exact/raw-file/raw-time overlap all zero", "Canonical?": "supplementary canonical sensitivity", "Result": "collapse flag=" + str(strict_collapse), "Risk": "C4/C5 test N reduced to 840 by two-repeat grouping", "Status": "PASS_WITH_LIMITATION" if strict_collapse else "PASS"},
    ...
    lines += ["", "## Decision-gate answers", "", "1. Ordinary source-only FL inadequacy: supported for these canonical fixed endpoints, with supervision/optimizer limitations disclosed.", f"2. Canonical equal-label A0T: {'PASS' if a0t_complete else 'BLOCKED'}.", "3. GAPS versus A0T claim strength: use only the observed per-target deltas above; do not generalize beyond the fixed seed/protocol.", "4. Routing-to-regression propagation: supported descriptively by the S_ALL-S_CC gap and misroute slices.", "5. 83D to 84D: modest overall average benefit; not statistically significant at the grouped-bootstrap 95% interval; retain C4 degradation.", "6. QC versus same-coverage random: positive at HC90 and HC95; capture analysis shows it does not catch every misroute.", f"7. Strict non-overlap: {'collapse flag raised; treat canonical window-level evidence as limited' if strict_collapse else 'conclusion retained without preregistered collapse flag'}.", "8. Calibration-budget wording: only ‘a predefined approximately 20% commissioning calibration set’; no few-shot claim.", "9. Pi 5: package hash chain is consistent with the canonical A4+R84+QC deployment.", "10. New algorithms: not required; stop algorithm exploration.", "", "This report does not authorize manuscript-number edits, figure regeneration, model changes, hyperparameter search, outlier deletion, or additional algorithms."]
