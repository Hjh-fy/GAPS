...
P0A checkpoint 是只读外部输入。执行前复制到新结果根的 `inputs/`，复制前后均校验 SHA-256，不修改旧结果。已核验源文件：
...
若复制后 tensor/key/shape/dtype 或 SHA-256 不一致，所有依赖该 checkpoint 的 E1 FedAvg 评价和 E2 均 fail closed。
...
- Target test：sealed；只有固定训练轮数或固定 100 adaptation steps 全部完成后才允许最终评价。
...
- Seed：42 only。
- Metrics：accuracy、macro_f1、per-class precision/recall/F1、confusion matrix、NLL、ECE、commissioning/training seconds、source retention、通信量。
...
E2 明确禁止：target CE、target class/phase labels、class-conditional CORAL、class-wise MMD、phase-conditioned MMD、same-class-phase stage MMD、conditional adversarial mask、target prototype anchor、pseudo labels，以及任何 GAPS-internal 条件化语义匹配。
...
- target calibration labels/phase 仅可进入 GAPS 明确注册的条件化 server-DA 分支，不得进入 target CE 或 test selection。
