...
| `gaps_deploy/final_runtime.py` docstring | 2026-07-14 | C12→C345、R3aK16、AutoV2、旧 QC wrapper | `MAINTAINED_LEGACY_PACKAGE_WRAPPER` | 文件名容易被误认为正式 C5 Runtime-v4 或最终 v5 唯一入口 | 不改 runtime 代码；正式 C5 Runtime-v4 baseline API 固定为 `C5H8Runtime`，本文件只保留 C12→C345 package compatibility |
...
- `gaps_deploy/final_runtime.py`：maintained legacy C12→C345 package wrapper；既不是正式 C5 Runtime-v4 唯一入口，也不是 v5。
- `gaps_deploy/c5_h8_runtime.py`：`C5H8Runtime` 是 formal C5 Runtime-v4 selective-output baseline API。
