...
| CLS-RUN-002 | 2026-07-11 | The refrozen v2 core screen can execute on the exact cloud-edge runtime | Runtime commit `ca2bcf0`; 39 v2 manifests bound to that revision | `python scripts/run_iotj_classification_cloud_edge.py --pi-hosts gaps@192.168.31.184` | `results/iotj_classification_ablation_20260711_v2_controller` | waiting-for-Pi | ECS runtime and all 39 manifests synchronized; local/ECS SHA256 matches for `client.py`, `task.py`, and `server_app.py`; ECS C5 calibration and idle checks pass. Controller PID 33856 waits every 60 seconds for the sole current Pi address. No Pi sync, tunnel, server, client, or training run has started yet. |
