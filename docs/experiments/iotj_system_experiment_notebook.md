...
| 3. Regression and expert selection | in progress | Frozen classifier outputs | C5-only H2.3+/H8 evaluators are ready; deployment-visible selector risk and unified R0-R7 leakage tests must be completed before formal P4 |
...
| CLS-RUN-002 | 2026-07-11 | The refrozen v2 core screen can execute on the exact cloud-edge runtime | Runtime commit `2424071`; frozen v2 manifests carry the same full revision | `python scripts/run_iotj_classification_cloud_edge.py --pi-hosts gaps@192.168.31.184` | `results/iotj_classification_ablation_20260711_v2r1_controller` and ECS `results/iotj_classification_ablation_20260711_v2r1` | in-progress | Controller PID 34972 is live. A0 completed and was recovered at 15:38 with PC exit 0. At 16:49 A0T was at round 18/25; ECS server and both physical clients were running, Pi reported 46.6 C and `throttled=0x0`. These are execution facts, not target-paper metrics. |
...
| 2026-07-11 | High safety | QC returns accept when no policy matches and treats missing score/threshold as zero/infinity | Final bundle validator must fail closed on missing policy, calibration references, or required risk fields. Do not advertise deployment readiness before this is fixed and parity-tested. |
...
1. Let the immutable v2r1 core queue continue on the real ECS/Pi/PC topology. Monitor controller health, Pi throttling, completion/recovery, run configs, and final checkpoint hashes; do not edit runtime losses mid-queue.
2. After A0/A0T/A2/A3/A4/A4S/A5/A6/A7 seed-42 complete, evaluate only round-25 C5 streams and review causal coherence before launching any extra seed. In particular compare A0 vs A0T, A4 vs A4S, A5 vs A6 vs A7, and A6 vs A7 for the current stage term.
3. If the core screen is coherent, run seeds 43-46 for A0/A0T/A4/A4S/A5/A7. Run `A7-no*` only if A7 merits detailed attribution. If the current MMD/stage definitions look harmful, create separately named corrected groups instead of rewriting v2 artifacts.
4. Before formal P4, replace the leaked legacy risk with a newly named deployment-visible selector risk and add invariance/leakage tests. Then rebuild C5-only Ridge/MLP/H2.3+/H8/simple-gate/P4 streams from the confirmed classifier, select one candidate on calibration-validation only, and lock C5 test evaluation.
...
- Runtime/QC audit: the current final runtime still serves the old C12->C345 bundle and QC can fail open when policy/schema fields are absent. Both are blocking items before a “closed-loop deployed system” claim.
