...
| 2026-07-11 | High integration, resolved 2026-07-23 | New C5 H2.3+/H8/selector artifacts are not loaded by historical `gaps_deploy/final_runtime.py` | Resolved without modifying the historical C3/C4/C5 runtime: `gaps_deploy/c5_h8_runtime.py` now strict-loads B5, H2.3, fixed R4/H8, risk and HC95/HC90. Formal 1,360-row reports are `equivalent`, with zero class/QC/auto-output mismatches. |
| 2026-07-11 | High safety, resolved 2026-07-23 | QC returned accept when no policy matched and treated missing score/threshold as zero/infinity | Generic fail-closed behavior was resolved in `6b6d930`; the dedicated C5/H8 runtime additionally requires all hashed risk assets, rejects malformed/non-finite inputs, and emits `auto_output_ppm` only for accept. HC95 and HC90 parity both pass 1,360 rows. |
| 2026-07-23 | High provenance, resolved | Runtime-contract v2 used the wrong dataset root; v3 corrected the root but did not hash-bind numeric phase labels | v2 is `blocked`, v3 is `superseded`, and v4 is the sole formal contract. v4 binds windows, metadata, phase labels, HC95/HC90 references and the audited bundle; no model was trained or refit. |
