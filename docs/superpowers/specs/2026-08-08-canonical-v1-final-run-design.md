...
2. Train the final A4 GAPS router (`ce_stats`, client semantic/replay disabled, selective aggregation disabled, frozen A4 server DA) for 25 rounds with local-epoch 1 from scratch; all checkpoint reuse flags are false. This preserves the prior frozen A4 training protocol so preprocessing is the main changed factor. Adapt only using target calibration; test remains sealed.
